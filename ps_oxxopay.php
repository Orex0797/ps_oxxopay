...
3
{
    ...
    {
        Configuration::updateValue('PS_OXXOPAY_SETTINGS', true);
        if (!parent::install()
    ...
    public function hookPaymentOptions($params)
{
    ...
    $this->smarty->assign([
        'oxxopay_reference' => Configuration::get('PS_OXXOPAY_REFERENCE'),
        'oxxopay_info' => Configuration::get('PS_OXXOPAY_INFO'),
    ]);

    $option = new PaymentOption();
    $option->setModuleName($this->name)
        ->setCallToActionText($this->trans('Pagar en OXXO', [], 'Modules.OxxoPay.Shop'))
        ->setAction($this->context->link->getModuleLink($this->name, 'validation', [], true))
        ->setAdditionalInformation($this->fetch('module:oxxopay/views/templates/hook/payment_info.tpl'));
